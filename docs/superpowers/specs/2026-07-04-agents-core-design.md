...
## Rolling narrative (breaks the moment-only lens)

The pipeline's narrative is structurally moments-only (a projection of a projection). The digest agent fixes half of that by having read the transcript — but a single retrospective blob still discards the rolling working notes written while reading. So the notes become product: after each sitting, the agent emits a **sitting narrative** (2–4 sentences + open threads, grounded in the full read of that sitting) as part of its output (`sittingNarratives: [{sittingIndex, summary, openThreads}]`). Persistence rides the already-planned `digest:sitting` events (digestion-zoom design, Part A): the sitting narrative IS the event's summary, stamped at the sitting's start time — the journal reads as a story along the timeline. The final session narrative is synthesized from sitting narratives + moments (compositional, auditable per sitting against its event range). Sub-moment texture (explorations that never crystallized into a moment) reaches the narrative because sitting summaries are written from the read, not from the moment list.
...
- **Confidence should be derived, not emitted** (post-rewrite A5: still ~95% "high" despite the rubric). ~~Experiment E2 (prompt/contract)~~ — resolved deterministically 2026-07-04 (derived confidence shipped in `src/pipeline/understand/derive-confidence.ts`; experiment unnecessary). Rule: `verification==="supported"` OR anchored+user → high; any anchored → medium; none → low. Both pipeline and agent arms apply `applyDerivedConfidence` after their final moments are set.
