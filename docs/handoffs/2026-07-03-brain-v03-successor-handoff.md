...
5. ~~**Cron digestion, configurable epochs**~~ **done** (`28bbf0e`) — in-process scheduler in the web server: GET/PUT `/api/digest/schedule` (enabled/intervalMinutes/debounceMinutes, persisted to `.intent/digest-schedule.json`), settings UI in the Digest page ("the press schedule"), mtime-quiet debounce, `digest:run` events. Open remainder: a headless `digest-watch` CLI for running without the web server, if ever needed. Caveat stands: resumed already-digested sessions won't re-digest.
