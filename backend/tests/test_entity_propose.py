...
def test_seed_skips_names_already_on_the_map(ws, adapter):
    ...
    from quire.entity_graph import decide
...
def test_inbox_flow_over_http(ws, tmp_path):
    ...
    seed_report = seed_proposals(
        ws,
        __import__("quire.adapters.fixture", fromlist=["FixtureWorkspace"]).FixtureWorkspace(ws),
...
def test_docs_classified_as_doc_kind_and_counted_separately():
    from quire.entity_propose import _is_doc_path, _question
...
def test_proposals_api_reports_coverage_and_already_held(ws, tmp_path):
    ...
    from quire.entity_graph import Attach, CreateEntity, GraphDiff, append_proposals
...
def test_wrong_scale_is_the_corpus_share_rule():
    ...
    five rounds running)."""
    from quire.entity_propose import _wrong_scale
