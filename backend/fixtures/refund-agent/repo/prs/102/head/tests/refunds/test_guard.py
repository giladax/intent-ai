...
def test_guard_blocks_above_ceiling():
    ...
    request = RefundRequest(customer_id="c1", tier="premium", risk="low", amount=101.0)
...
def test_guard_allows_up_to_new_ceiling():
    guard = RefundToolGuard()
    request = RefundRequest(customer_id="c1", tier="premium", risk="low", amount=100.0)
    guard.check(request, approved_decision())
