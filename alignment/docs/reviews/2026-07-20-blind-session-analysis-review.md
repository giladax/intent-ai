...
Scope reviewed: `quire_align/session.py`, its render wiring in `model.py` (`_around_session/_check/_entity`), `cli.py digest-session`, propagation in `story.py`/`relevance.py`, `tests/test_session.py`; and `quire_align/analysis/{graph,nodes,classify,evidence,matching,coverage,llm,prompts}.py`.
