...
claim-by-claim against `quire_align/`, plus a dead-code hunt the specs
...
### 1. `quire_align/static/mirror.html` — orphaned page (160 lines)

**Proof it was dead:**
- `grep -rn "mirror.html"` across `quire_align/ tests/ evals/ docs/specs/`
...
**Removed:** `git rm quire_align/static/mirror.html`. Tests stay green
(201).

### 2. `quire_align/analysis/config.py:78` — dead `DEFAULT` name binding

**Proof it was dead:** `grep -rnw "DEFAULT" quire_align tests evals`
