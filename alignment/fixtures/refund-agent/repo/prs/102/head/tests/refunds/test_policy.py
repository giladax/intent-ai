...
def test_premium_low_risk_within_limit_auto_approved():
    decision = RefundPolicy().evaluate(make_request(amount=100.0))
    assert decision.approved


def test_premium_above_limit_escalates():
    decision = RefundPolicy().evaluate(make_request(amount=101.0))
...
def test_high_risk_premium_at_new_limit_still_escalates():
    decision = RefundPolicy().evaluate(make_request(risk="high", amount=100.0))
    assert not decision.approved
    assert decision.requires_human_approval
